...
old_section = '''    # 数据库直连配置（部署到云服务器后使用）
    ...
    with st.sidebar.expander("配置 Gllue 数据库", expanded=False):
        ...
        if st.button("🚀 从数据库同步", type="primary", use_container_width=True):
            if not all([db_host, db_name, db_user, db_pass]):
                st.error("请填写完整的数据库连接信息")
            else:
                with st.spinner("正在从数据库同步..."):
                    try:
                        ...
                        st.success(f"数据库同步完成！Offer: {stats['offers_fetched']}, Forecast: {stats['forecasts_fetched']}")
                        st.rerun()
                    except Exception as e:
...
if old_section in content:
    ...
    print("Database section replaced successfully")
else:
    print("WARNING: Could not find old database section")
